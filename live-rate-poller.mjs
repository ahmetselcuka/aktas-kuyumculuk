...
function computeChangePercent(previousValue, currentValue) {
  ...
  return `${sign}${safeValue.toFixed(2).replace(".", ",")}`;
}

function enrichRatesWithComputedChange(rates, previousPayload) {
  const previousRates = Array.isArray(previousPayload?.rates) ? indexRates(previousPayload.rates) : {};

  return rates.map((rate) => {
    const previousRate = previousRates[rate.key];
    const previousSell = parseNumber(previousRate?.sell);
    const currentSell = parseNumber(rate.sell);
    const computedChange = computeChangePercent(previousSell, currentSell);
    const computedTrend = currentSell > previousSell ? "up" : currentSell < previousSell ? "down" : "flat";

    return {
      ...rate,
      trend: Number.isFinite(previousSell) ? computedTrend : rate.trend,
...
async function runOnce() {
  const previousPayload = await readExistingFirebasePayload();
  let payload;
  ...
  payload = {
    ...payload,
    rates: enrichRatesWithComputedChange(payload.rates, previousPayload)
  };
